  ...
  return (
    <main className="night-shell" style={{ backgroundImage: `linear-gradient(rgba(10, 24, 45, .35), rgba(10, 24, 45, .72)), url(${BACKGROUND})` }}>
      ...
      {phase === "playing" && <div className="placed-item-dom-layer" aria-live="polite" aria-label={`設置中の防衛道具 ${placedItems.length}個`}>{placedItems.map((item) => {
        ...
        return <div key={`${item.id}-${item.x}-${item.y}`} className={`placed-item-dom placed-item-${item.id} ${tongueActive ? "is-striking" : ""}`} style={effectStyle}><span className="placed-item-range" aria-hidden="true" /><span className="placed-item-art" aria-hidden="true" />{tongueActive && <span key={frogTongue.nonce} className="frog-tongue" style={tongueStyle} aria-hidden="true" />}<span className="item-runtime-ring"><i>{item.duration === null ? "∞" : Math.ceil(item.remaining ?? 0)}</i></span></div>;
      })}</div>}
