  ...
  return (
    <main className={`night-shell stage-${difficulty}`}>
      ...
      {phase === "playing" && <div className="placed-item-dom-layer" aria-live="polite" aria-label={`設置中の防衛道具 ${placedItems.length}個`}>{placedItems.map((item) => {
        ...
        return <div key={item.key} data-babylon-underlay={item.underlayDisabled ? "disabled" : "enabled"} className={`placed-item-dom placed-item-${item.id} ${frogActive ? "is-aiming" : ""} ${tonguePulling ? "is-striking" : ""}`} style={{ ...effectStyle, ...tongueStyle }}><span className="placed-item-art" aria-hidden="true" />{item.id === "incense" && <><svg className="incense-realistic" viewBox="0 0 64 64" aria-hidden="true"><defs><linearGradient id="incenseCoil" x1="0" y1="0" x2="1" y2="1"><stop stopColor="#789a47"/><stop offset=".5" stopColor="#365b32"/><stop offset="1" stopColor="#1f3728"/></linearGradient></defs><ellipse className="incense-plate-rim" cx="32" cy="43" rx="26" ry="10"/><ellipse className="incense-plate-inner" cx="32" cy="40" rx="23" ry="8"/><path className="incense-coil-shadow" d="M41 26c0-8-6.5-14.5-14.5-14.5S12 18 12 26s6.5 14.5 14.5 14.5c10 0 18-8 18-18s-8-18-18-18c-11.6 0-21 9.4-21 21S14.9 46.5 26.5 46.5c13 0 23.5-10.5 23.5-23.5"/><path className="incense-coil-path" d="M41 26c0-8-6.5-14.5-14.5-14.5S12 18 12 26s6.5 14.5 14.5 14.5c10 0 18-8 18-18s-8-18-18-18c-11.6 0-21 9.4-21 21S14.9 46.5 26.5 46.5c13 0 23.5-10.5 23.5-23.5"/><circle className="incense-ember-core" cx="50" cy="23" r="3.7"/><circle className="incense-ember-glow" cx="50" cy="23" r="6.2"/></svg><span className="incense-smoke" aria-hidden="true" /></>}{item.id === "frog" && <span className={`frog-mouth ${frogActive ? "is-open" : ""}`} aria-hidden="true" />}{tonguePulling && <span key={activeFrogTongue?.nonce ?? 0} className="frog-tongue" aria-hidden="true" />}<span className="item-runtime-ring"><i>{item.duration === null ? "∞" : Math.ceil(item.remaining ?? 0)}</i></span></div>;
      })}</div>}
