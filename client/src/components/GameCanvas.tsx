  ...
  return (
    <main className="night-shell" style={{ backgroundImage: `linear-gradient(rgba(10, 24, 45, .35), rgba(10, 24, 45, .72)), url(${BACKGROUND})` }}>
      ...
      {phase === "playing" && <div className="placed-item-dom-layer" aria-live="polite" aria-label={`設置中の防衛道具 ${placedItems.length}個`}>{placedItems.map((item) => {
        ...
        return <div key={item.key} data-babylon-underlay={item.underlayDisabled ? "disabled" : "enabled"} data-item-motion={item.id === "cat" ? "beckoning" : "static"} className={`placed-item-dom placed-item-${item.id} ${frogActive ? "is-aiming" : ""} ${tonguePulling ? "is-striking" : ""}`} style={{ ...effectStyle, ...tongueStyle }}><span className="placed-item-range" aria-hidden="true" /><span className="placed-item-art" aria-hidden="true" />{item.id === "cat" && <span className="cat-waving-paw" aria-hidden="true" />}{item.id === "incense" && <span className="incense-coil" aria-hidden="true" />}{item.id === "frog" && <span className={`frog-mouth ${frogActive ? "is-open" : ""}`} aria-hidden="true" />}{tonguePulling && <span key={activeFrogTongue?.nonce ?? 0} className="frog-tongue" aria-hidden="true" />}<span className="item-runtime-ring"><i>{item.duration === null ? "∞" : Math.ceil(item.remaining ?? 0)}</i></span></div>;
      })}</div>}
