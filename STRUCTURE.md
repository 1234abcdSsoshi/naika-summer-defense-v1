...
client/src/game/difficulty.ts         # 3難易度プロファイルとプレイ精度に応じる適応脅威値
client/src/game/telemetry.ts          # 匿名ローカルイベント、CustomEvent、結果集計
...
現行プロトタイプでは、`GameWorld` がBabylonノードと状態を所有し、ReactはUIとエンジンのライフサイクルだけを扱う。重要な不変条件は、ゲーム状態が `Playing` 以外の時にシミュレーションを進めないこと、キャンバスへのポインタ入力をdispose時に必ず解除すること、開始ユーザー操作後のみBGMを再生すること、分析イベントは端末内のゲーム行動だけを匿名で記録し個人情報を含めないことの4点である。
